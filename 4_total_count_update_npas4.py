...
def main():
    ...
    summary = {}  # {(folder_id, region): {npas4, pv, coexp}}
    ...
    for root, _, files in os.walk(top):
        for fname in files:
            ...
            if key not in summary:
                summary[key] = {'npas4': 0, 'pv': 0, 'coexp': 0, 'area_px2': area}
    ...
    for (folder_id, region), counts in summary.items():
        records.append({
            'Folder': folder_id,
            'Regions': region,
            'npas4': counts.get('npas4', 0),
            'PV': counts.get('pv', 0),
            'Co-expression': counts.get('coexp', 0),
            'Area_px2': counts.get('area_px2')
        })

    df = pd.DataFrame(records, columns=['Folder', 'Regions', 'npas4', 'PV', 'Co-expression'])

    # Sort by defined region order
    region_order = ['bla', 'il', 'dhpc', 'vhpc']
